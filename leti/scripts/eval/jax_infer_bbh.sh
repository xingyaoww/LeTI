...
# if split == "test", then use the test set
if [[ $SPLIT == "test" ]]; then
  DATA_DIR=data/processed/eval/bbh
  EVALUATE_TASK="bbh"
  MAX_LENGTH=1536
elif [[ $SPLIT == "test-cot" ]]; then
  DATA_DIR=data/processed/eval/bbh_cot
  EVALUATE_TASK="bbh_cot"
  MAX_LENGTH=2048
else
...
INFER_DATA=$DATA_DIR/test_prompts.json
...
target_not_exists=$(gsutil -q stat $GS_OUTPUT_DIR/generations.json || echo 1)
if [[ $target_not_exists == 1 ]]; then
  sync_from_gs $DATA_DIR
  sync_from_gs $HF_MODEL_DIR
  echo "Running inference since $GS_OUTPUT_DIR/generations.json does not exist"
  python3 leti/scripts/eval/jax_infer.py \
    ...
    --postprocess_type "bbh" \
    --disable_execution \
    $EXTRA_ARGS

    python3 leti/scripts/eval/metric/calculate_bbh_metric.py $GS_OUTPUT_DIR
fi
