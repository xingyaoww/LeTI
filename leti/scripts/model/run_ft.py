...
This code is used for fine-tuning baseline models for comparison.
...
from leti.utils.reward import (
    get_reward_mask_and_value,
    visualize_rm_example
)
...
class DataTrainingArguments:
    ...
    is_reward_model: bool = field(
        default=False, metadata={"help": "Whether or not to train a reward model."}
    )
...
def main():
    ...
    eval_batch_size = train_batch_size # TODO: support different eval batch size

    def tokenize_function(examples, batch_size=train_batch_size):
        with CaptureLogger(tok_logger) as cl:
            if config.is_reward_model:
                text_a = examples["text_a"]
                # add bos and eos to text_a (language feedback)
                ...
                # concatenate text_a and text_b
                text = [f"{txt_a}{txt_b}" for txt_a, txt_b in zip(text_a, text_b)]

                # parse reward_offset_mapping from string to dict mapping
                reward_offset_mapping: List[Mapping[Tuple[int, int], Union[float, None]]
                    ...
                    for mapping_str in examples["reward_offset_mapping"]
                ]

                # use (character) length of text_a as prefix_length for offset_mapping
                # so the reward_offset_mapping refers to spans in [text_a + text_b] (i.e. text)
                prefix_length = [len(txt) for txt in text_a]
                reward_offset_mapping = [
                    {
                        (k[0] + cur_prefix_len, k[1] + cur_prefix_len): v
                        for k, v in offset_mapping.items()
                    }
                    for offset_mapping, cur_prefix_len in zip(reward_offset_mapping, prefix_length)
                ]
                ...
                output = tokenizer(
                    ...
                    truncation=True,
                    return_tensors="np",
                    pad_to_multiple_of=256, # larger value to avoid repetitive compilation
        ...
        if config.is_reward_model:
            reward_mask, reward_val = get_reward_mask_and_value(
                shape=output["input_ids"].shape,
                token_offset_mapping=output["offset_mapping"],
                reward_offset_mapping=reward_offset_mapping,
            )
            output["reward_mask"] = reward_mask
            output["reward_val"] = reward_val
            output.pop("offset_mapping") # not needed anymore
        else:
    ...
    lm_datasets = tokenized_datasets # we don't need to group texts
    # sample few examples to print
    # visualize_rm_example
    if config.is_reward_model:
    ...
    if training_args.do_train:
        ...
        # print the distribution of the dataset sequence length
        seq_lens = np.array([len(x) for x in train_dataset["input_ids"]])
        print("Train dataset seq len distribution:") # value counts
    ...
    if training_args.do_eval:
        ...
        print("Eval dataset seq len distribution:") # value counts
        seq_lens, len_count = np.unique(seq_lens, return_counts=True)
        for i in range(len(seq_lens)):
            print(f"- {seq_lens[i]}: {len_count[i]}")
    ...
    if training_args.do_eval:
        ...
        desc = (
            f"Step... ({0} | Eval Loss: {eval_metrics['lm_loss']} | Eval Perplexity:"
        ...
        for step in tqdm(range(1, steps_per_epoch + 1), desc="Training...", position=1, leave=False):
            ...
            if cur_step % training_args.logging_steps == 0 and cur_step > 0:
                ...
                if jax.process_index() == 0:
                    ...
                    wandb.log(
                        {
                            "train/lm_loss": train_metric["lm_loss"],
                            "train/learning_rate": train_metric["learning_rate"],
                ...
                epochs.write(
                    f"Step... ({cur_step} | Loss: {train_metric['lm_loss'].mean()}, Learning Rate:"
            ...
            if training_args.do_eval and cur_step % training_args.eval_steps == 0 and cur_step > 0:
                ...
                # Print metrics and update progress bar
                desc = (
                    f"Step... ({cur_step} | Eval Loss: {eval_metrics['lm_loss']} | Eval Perplexity:"
                    f" {eval_metrics['perplexity']})"
                )
                epochs.write(desc)
                epochs.desc = desc
    ...
    if training_args.do_eval:
        ...
        if jax.process_index() == 0:
            eval_metrics = {
                f"final_eval/{metric_name}": value
