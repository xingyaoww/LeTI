...
def convert_hf_weight_to_t5x_ckpt(
    model: PreTrainedModel,
    model_params: dict,
    gs_output_dir: str,
    num_partitions: int
):
    # NOTE: model_params should be a dict of jax arrays (actual HF model weights), NOT t5x train state

    def _init_state(rng: jax.random.KeyArray, *unused_args, **unused_kwargs) -> flax.core.scope.FrozenVariableDict:
    ...
    partitioner = PjitPartitioner(num_partitions) # default partitioning
    ...
    # Put params according to the partitioning spec
    params = jax.tree_map(
        lambda param, spec: jax.device_put(param, NamedSharding(partitioner.mesh, spec)),
        model_params,
        unfreeze(params_spec)
    )
    ...
    print("Saving model ckpt to", gs_output_dir)
...
def hf_from_pretrained(
    ...
    custom_process_state_fn: Callable = None,
    ...
    **kwargs
    ...
    # NOTE: different compared to .from_pretrained() in huggingface/transformers
    ...
    if missing_keys and not _do_init:
        logger.warning(
            f"The checkpoint {pretrained_model_name_or_path} is missing required keys (ignored missing 'params_axes'): {missing_keys_wo_params_axes}. "
    ...
    if len(missing_keys) > 0:
        logger.warning(
            ...
            f" {pretrained_model_name_or_path} and are newly initialized (ignored missing 'params_axes'): {missing_keys_wo_params_axes}\nYou should probably"
